...
function allGameLoop(timeStamp){
    ...
    if(platforms.length !== 0){
        platforms.forEach((platform) => {
            platform.update();
            platform.draw();
            if((player.going || platform.alpha < 0.3) || (platform.x > canvas.width * 0.2) && !willStillCheck) return;
    ...
    if(timer > timeInterval){
        ...
        timer = 0;
        timeInterval = Math.floor(Math.random() * (500 - 200) + 200);
    }
...
function createRandomPlatform(){
    ...
    platforms.push(new Platform(CANVAS_WIDTH, Math.round(Math.random() * CANVAS_HEIGHT), Math.floor(Math.random() * (maxWidth - 250) + 250), 64));
}
